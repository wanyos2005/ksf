<?php

/**
 * This is the model class for table "{{months}}".
 *
 * The followings are the available columns in table '{{months}}':
 * @property integer $id
 * @property string $name
 * @property string $abbr
 */
class Months extends CActiveRecord
{
	/**
	 * Returns the static model of the specified AR class.
	 * @param string $className active record class name.
	 * @return Months the static model class
	 */
	public static function model($className=__CLASS__)
	{
		return parent::model($className);
	}

	/**
	 * @return string the associated database table name
	 */
	public function tableName()
	{
		return 'tbl_months';
	}

	/**
	 * @return array validation rules for model attributes.
	 */
	public function rules()
	{
		// NOTE: you should only define rules for those attributes that
		// will receive user inputs.
		return array(
			array('name, abbr', 'required'),
			array('name', 'length', 'max'=>20),
			array('abbr', 'length', 'max'=>5),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, name, abbr', 'safe', 'on'=>'search'),
		);
	}

	/**
	 * @return array relational rules.
	 */
	public function relations()
	{
		// NOTE: you may need to adjust the relation name and the related
		// class name for the relations automatically generated below.
		return array(
		);
	}

	/**
	 * @return array customized attribute labels (name=>label)
	 */
	public function attributeLabels()
	{
		return array(
			'id' => 'ID',
			'name' => 'Name',
			'abbr' => 'Abbr',
		);
	}

	/**
	 * Retrieves a list of models based on the current search/filter conditions.
	 * @return CActiveDataProvider the data provider that can return the models based on the search/filter conditions.
	 */
	public function search()
	{
		// Warning: Please modify the following code to remove attributes that
		// should not be searched.

		$criteria=new CDbCriteria;

		$criteria->compare('id',$this->id);
		$criteria->compare('name',$this->name,true);
		$criteria->compare('abbr',$this->abbr,true);

		return new CActiveDataProvider($this, array(
			'criteria'=>$criteria,
		));
	}
	
	private static $_months=array();
		//Display all departments
	public static function show_months()
    {
        //if(!isset(self::$_items[$type]))
            self::load_months();
        return self::$_months;
    }
	  public static function get_months($id)
    {
        if(!isset(self::$_months))
            self::load_months();
        return isset(self::$_months[$id]) ? self::$_months[$id] : false;
    }
 
	  private static function load_months()
    {
        self::$_months=array();
        $models=self::model()->findAll(array(
            'order'=>'id',
        ));
        foreach($models as $model)
            self::$_months[$model->id]=$model->name;
    }
 public static function getmonthname($id){
 
 				$criteria3 = new CDbCriteria;
				$criteria3->condition = 'id=:id';
				$criteria3->params = array(':id' => $id);
				$ourfishes3 = self::model()->findAll($criteria3);
			
			foreach ($ourfishes3 as $fish3):
			return $fish3->name;
			endforeach;
 
 }

}