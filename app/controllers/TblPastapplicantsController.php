...
class TblPastapplicantsController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('index','view','SaveStud','PrintSubform','GeneratePdf','List'),
				'users'=>array('*'),
			),
	...
	public function actionSaveStud()
	{
		...
		if(isset($_POST['FunzoPersonalDetails']))
		{
			...
			$tblPastapplicants->TELNO=$model->TEL_NUMBER;
			...
			$tblPastapplicants->CONST_C=$model->CONSTITUENCY_IDFK;
			$tblPastapplicants->YEAR_OF_STUDY=1;
			$tblPastapplicants->STUD_COUNTY=$model->COUNTY_IDFK;
			$tblPastapplicants->ACADEMIC_YEAR='2014/2015';
...
//$html2pdfPath = Yii::getPathOfAlias('ext.tcpdf.tcpdf');
		//  Yii::app()->request->baseUrl.'/images/helb_logo.jpg';
$html2pdfPath = Yii::createComponent('booster.TcPdf',
                            'P', 'cm', 'A4', true, 'UTF-8');
...
$pdf->Output('funzoKenya.pdf', 'I');
	...
	//$model = CrePayouts::model()->findAll('PRINTDATE=:PRINTDATE && REF=:REF && UNCODE=:UNCODE',array(':PRINTDATE'=>$date,':REF'=>1,':UNCODE'=>$uni));
 		$model = CrePayouts::getpaymentlist($date,$uni);		
		$this->render('//crePayouts/paymentlist',array(
			'items'=>$model,
		));
	}
